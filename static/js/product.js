class Product{
    ...
    static async requestProduct(uri, iteration){
        ...
        let response = await fetch(`/post/product/${uri}`, {method: "POST"});
        let text = await response.text();
        this.json= JSON.parse(text);
        if(iteration){
            for(let id in this.json){
                if(id !== "relleno"){
                    let product = Product.createContentProduct(this.json[id]);
    ...
    static createContentProduct(json){
        return `
            <article class="product" id="${json["id"]}" data-classification="${json["classification"]}">
                <article>
                    <div id="cont-img">
...
function initProduct(){
    ...
    let id = location.search.match(/((([a-zA-Z0-9])*-[0-9])$)/i);
    if(clasification){
        ...
        Product.requestProduct(("serch-product/" + id[0]), false);
        setTimeout(Product.stopLoader, 1000);
    }
}
function callProduct(){
    Product.requestProduct("list-product", true);
    if(index >= 2){
        Product.stopLoader();
    }else{
        setTimeout(()=>{
